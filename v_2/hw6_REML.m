...
%  disp(beta_new');
  ...
  % Calculate the M_star
  M_star = eye(N) - X*inv(X'*inv(Vy)*X)*X'*inv(Vy);
  ...
  for t = 1:T
    for m = 1:T
      Omega(t, m) = trace(inv(Vy)*M_star*V{t}*M_star'*inv(Vy)*V{m});
    endfor
  endfor

  for t = 1:T
    Rho(t, 1) = (y-X*beta_old)'*inv(Vy)*M_star*V{t}*M_star'*inv(Vy)*(y-X*beta_old); % Here is a problem with the iteration 
  endfor
